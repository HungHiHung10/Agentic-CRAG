...
def _unique_provider_chain() -> List[str]:
...
def _build_api_chat_model(provider: str, max_tokens: int):
    try:
...
def _load_api_llm(max_tokens: int, purpose: str):
    configured_models: List[Tuple[str, str, object]] = []
    for provider in _unique_provider_chain():
        try:
            model_name = _resolve_model(provider)
            model = _build_api_chat_model(provider, max_tokens=max_tokens)
        except Exception as exc:
    ...
    if not configured_models:
        providers = ", ".join(_unique_provider_chain()) or "<empty>"
...
def load_llm_model():
    return _load_api_llm(max_tokens=2048, purpose="generation")


def load_judge_model():
    return _load_api_llm(max_tokens=256, purpose="judge")